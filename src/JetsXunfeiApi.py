...
# 语音转文字
# 非实时转写调用demo
# To do： 转成exe之前，需要转为不可读取的格式，以便保护    APPID 和  SECRET_KEY
import os
import time
...
# Jet从讯飞申请到的：
# 2020-08-20 修改： 从class外定义的全局变量，改为本class的属性，以便在外部调用时，隐藏此属性
# 需要购买：https://www.xfyun.cn/services/lfasr
APPID = "5ae46872" 
...
# 原名RequestApi
class XunfeiAudioConvertRequestApi(object):
    '''
    ...
    # 2020-08-20 修改： 从class外定义的全局变量，改为本class的属性，以便在外部调用时，隐藏两个需要保密的属性
    def __init__(self, upload_file_path, speaker_number=2):
    #def __init__(self, appid, secret_key, upload_file_path, speaker_number=2):
        """ Jet改写的构造函数，添加了后两个参数
        ...
        Args:
            appid: 来自从讯飞注册的内容
            secret_key: 来自从讯飞注册的内容
            upload_file_path: 要转写的音频文件mp3/wav
            speaker_number：音频文件中讲话者的人数
        """
        self.appid = APPID
        self.secret_key = SECRET_KEY
        self.upload_file_path = upload_file_path
        # 估算转写时间，返回分钟
        self.estimated_converting_time = getaudioinfo.calculate_estimated_converting_time(upload_file_path)
        ...
        self.speaker_number = speaker_number  # 转写语音中讲话人的个数，默认是2个

        self.has_GUI = False  # Jet: 默认不用GUI显示
        self.converting_thread = None
        # print("in XunfeiAudioConvertRequestApi __init__")

    def set_convert_thread(self, converting_thread):
        """  Jet adds: 设置转写的线程，

        Args:
            converting_thread: 传入的参数为一个已经创建好的转写线程
        ...
        if converting_thread is not None:
            ...
            # 根据不同的apiname生成不同的参数,本示例中未使用全部参数您可在官网
            # (https://doc.xfyun.cn/rest_api/%E8%AF%AD%E9%9F%B3%E8%BD%AC%E5%86%99.html)查看后选择适合业务场景的进行更换
    ...
    def gene_request(self, apiname, data, files=None, headers=None):
        ...
        if result["ok"] == 0:
            ...
            if "upload" in msg and self.cur_process < 6:
                self.cur_process += 2
            elif "merger 成功:" in msg:
                self.cur_process = 10  # Jet: 假设上传、合并文件占10%的时间
    ...
    def all_api_request(self):
        ...
        interval = 10  # Jet:指定间隔秒数
        time_elapsed = 0  # Jet: 已经花费的时间，之前的上传不算，这里只计算转写过程的时间

        update_ui_times = self.estimated_converting_time * (60 / 30)  # 按30秒刷新一次计算
        ...
        while True:
            ...
            if progress_dic['err_no'] != 0 and progress_dic['err_no'] != 26605:
                ...
                if self.has_GUI and time_elapsed % 30 == 0:
                    ...
                    msg = '转写任务编号 ' + taskid + ' 正在进行中... '  # Jet： 不用显示具体的信息

                    self.cur_process += int(80 / update_ui_times)  # Jet: 每次刷新占的进度百分比，按：上传和合并占10%, 转写占80%, 写文本文件10%分配
                    if self.has_GUI:
                        self.converting_thread.emit_msg(self.cur_process, msg)
        ...
        msg = "服务器端转写完成。"
        self.cur_process = 10 + 80  # Jet: 按：上传和合并占10%, 转写占80%, 写文本文件10%分配
        if self.has_GUI:
            self.converting_thread.emit_msg(self.cur_process, msg)
...
# 1个讲话者 GUI显示进度,
def convert_1speaker_audio_to_text_with_gui(audio_file, output_file, converting_thread):
    print('in xunfei Api, converting 1 speaker audio.')
    converting_result = False
    ...
    try:
        ...
        print(msg)
        api.cur_process = 100  # Jet: 按：上传和合并占10%, 转写占80%, 写文本文件10%分配
        converting_result = True
    except:
        msg = "语音转写内容保存失败!"
    finally:
        print(msg)
        if api.has_GUI:
            api.converting_thread.emit_msg(api.cur_process, msg)
        fw.close()
    return converting_result


# 2个讲话者 GUI显示进度
def convert_2speakers_audio_to_text_with_gui(audio_file, output_file, converting_thread):
    print('in xunfei Api, converting 2 speakers audio.')
    converting_result = False

    api = XunfeiAudioConvertRequestApi(audio_file, speaker_number=2)
    api.set_convert_thread(converting_thread)  # 设置刷新进度显示的线程
...
# 2个讲话者，无GUI
# 2020-08-20 remix, add: 显示用时
def convert_2speakers_audio_to_text(audio_file, output_file):
    start = datetime.datetime.now()
    print(f"开始转写{audio_file} " + start.strftime('%Y-%m-%d_%H_%M_%S'))

    api = XunfeiAudioConvertRequestApi(audio_file, speaker_number=2)
    result = api.all_api_request()

    # 指定编码格式为UTF-8格式，中英文都可正常存储
    with open(output_file, 'w', encoding='utf-8') as fw:  # w为写文件write
        fw.write(str(result))
        msg = "语音转写内容保存到文件:%s成功！" % output_file

        end = datetime.datetime.now()
        print(f"结束转写{audio_file} " + end.strftime('%Y-%m-%d_%H_%M_%S'))
...
# 2020-08 Jet： 直接用命令行执行，避免pyqt在转写过程中崩溃！
def main():
    #audio (mp3) to raw.txt
    #audio = r"C:\#1-Jet's Docs\教练\唯行堂-云对话-MCC教练秀\03-云对话-教练秀-客户泽宇教练"
    #dir = r"C:\#1-Jet's Docs\教练\唯行堂-云对话-MCC教练秀\"

    audio = r"C:\#1-Jet's Docs\教练\教练录音、录像\2020-1008-梦瑶-接纳自己-拥抱负面情绪"
    # "C:\#1-Jet's Docs\教练\其他教练录音-转写\Susan教练 - 20200917"

    #r"C:\#1-Jet's Docs\教练\唯行堂-云对话-MCC教练秀\18-个人成长-提升自信"
   # audio = r"C:\#1-Jet's Docs\教练\教练录音、录像\2-Jet被教练\2020-0925-仇龙教练Jet放慢工作、享受生活"
    # "C:\#1-Jet's Docs\教练\唯行堂-云对话-MCC教练秀\15-教练秀-从被动到主动（补船的漏洞）"

    #audio = r"C:\#1-Jet's Docs\教练\教练录音、录像\2020-0821-tanche-瞩目录制"
    audio_file = audio + ".mp3"
    raw_txt_file =audio + "_raw.txt"
    convert_2speakers_audio_to_text(audio_file, raw_txt_file)

    '''
    audio  =r"C:\#1-Jet's Docs\教练\教练录音、录像\2020-0820-谢辉-有关CP"
    audio_file = audio + ".mp3"

    raw_txt_file =audio + "raw.txt"
    convert_2speakers_audio_to_text(audio_file, raw_txt_file)
    '''
